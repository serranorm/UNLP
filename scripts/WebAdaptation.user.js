...
// @author      Diego Takahashi, Sebastian Pierini, Mauricio Witkin, Ramon Serrano
...
// Funcion que realiza la previsualizacion de la pagina diseñada.
function previewDocument(){
	hideOriginalDom();
	adaptationTemplate();
	createButtonMenuFlotant("Volver");
}

// Funcion que retorna al diseño original de la pagina.
function returnDocument(){
	var headTemp = $("head").children();
	var bodyTemp = $("body").children();
	var originalElements = $("[originalDoc]");
	$(headTemp).not("[originalDocHead]").remove();
	$(bodyTemp).not("[originalDoc]").remove();
	for (var i = 0; i < originalElements.length; i++) {
		var element = $("*[originalDoc=off]:eq("+i+")");
		$(element).css("display", ""+ $(element).attr("originalDisplay") +"");
		$(element).removeAttr("originalDisplay");
	}
	$("*[originalDoc]").removeAttr("originalDoc");
	$("*[originalDocHead]").removeAttr("originalDocHead");
	startEdit();
}

// Funcion que oculta el DOM original para realizar la previsualizacion del prediseñado.
function hideOriginalDom(){
	dinamicPreview = true;
	var headOriginal= $("head").children();
	$(headOriginal).attr("originalDocHead","off"); 
	var bodyOriginal= $("body").children().not("script");
	$(bodyOriginal).attr("originalDoc","off");
	for (var i = 0; i < bodyOriginal.length; i++) {
		var element = $("*[originalDoc=off]:eq("+i+")");
		$(element).attr("originalDisplay", ""+ $(element).css("display") +"");
	}  
	$(bodyOriginal).hide();
}

// Funcion que agrega el boton de previsualizacion
function createButtonMenuFlotant(operation){
	$("body").append("<div id='buttonMenuFlotant'> "+ operation + " </div>");
	$("#buttonMenuFlotant").css({"position": "fixed", "top":"60px", "right":"10px", "border-style":"ridge", "border-color":"#F5DEB3", "background-color":"#d8ffb3","width":"100px", "text-align":"center"});
	if(operation == "preview"){
		$("#buttonMenuFlotant").on("click", previewDocument);
	} else {
		$("#buttonMenuFlotant").on("click", returnDocument);
	}
}

// Funcion que elimina el boton de previsualizacion
function destroyButtonMenuFlotant(){
	$("#buttonMenuFlotant").remove();
}
...
function startEdit(){
	...
	if(!editIniciado){
		...
		$("*").not(elements).not(element_button_click).not("#BackgroundMenuButton").not("#buttonMenuFlotant").on("mouseenter", frameElement);
		$("*").not(elements).not(element_button_click).not("#BackgroundMenuButton").not("#buttonMenuFlotant").on("mouseleave", function(){deframeElement($(this));});
		//createButtonMenuFlotant("preview");
		createPreviewIFrame();
	}
}
...
function importElement(source, destination, iBody){
	...
	if ("pattern0" === source.pattern ) {
		...
		// Alto contraste
		divElement.css("background-color", "black");
